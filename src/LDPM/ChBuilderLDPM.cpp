...
void ChBuilderLDPM::read_LDPM_info(std::shared_ptr<ChMesh> my_mesh,  std::shared_ptr<ChMaterialVECT> vect_mat, std::string& LDPM_data_path, 
    ...
    if (nodesFile.is_open()) {
		...
		while (std::getline(nodesFile, line)) {			
			...
			if( line[0]!='/') {				
				...
				sline>> x >> y >> z;				
				//
				auto mnode= chrono_types::make_shared<ChNodeFEAxyzrot>(chrono::ChFrame<>(chrono::ChVector<>(x, y, z)));
	...
	*/	
    ...
    if (elemFile.is_open()) {       	
    	...
    	throw chrono::ChException("ERROR opening element info file: " + std::string(elemFilename) + "\n");
    	exit(EXIT_FAILURE);
    }
	...
	*/
	...
	//
	////////////////////////////////////////////////////////////////////////   	
	//double Mvertices[my_mesh->GetNelements()*36][3];
	//double Mvertices[2000*36][3];
	std::vector<ChVector<>> Mvertices;
	if (verticesFile.is_open()) {
		...
		while (std::getline(verticesFile, line)) {
			...
			if( line[0]!='/') {				
				...
				Mvertices.push_back(ChVector<>(x, y, z));
                		++idfacet;                		
    	...
    	throw chrono::ChException("ERROR opening vertices file: " + std::string(verticesFilename) + "\n");
    	exit(EXIT_FAILURE);
    }	
    
    /*
     for(int i=0; i<my_mesh->GetNelements()*36; i++){
    ...
    */
	...
	if (facetFile.is_open()) {
		...
		while (std::getline(facetFile, line)) {
			...
			if( line[0]!='/') {				
				...
				*/	
				...
				//
				/*
				auto mnode1= chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(Mvertices[int(facetvec[1])][0], 
				Mvertices[int(facetvec[1])][1], Mvertices[int(facetvec[1])][2])));
				auto mnode2= chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(Mvertices[int(facetvec[2])][0], 
				Mvertices[int(facetvec[2])][1], Mvertices[int(facetvec[2])][2])));
				auto mnode3= chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(Mvertices[int(facetvec[3])][0], 
				...
				*/
				...
				auto mnode1= chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(Mvertices[int(facetvec[1])].x(), 
				Mvertices[int(facetvec[1])].y(), Mvertices[int(facetvec[1])].z() )));
				auto mnode2= chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(Mvertices[int(facetvec[2])].x(), 
				Mvertices[int(facetvec[2])].y(), Mvertices[int(facetvec[2])].z() )));
				auto mnode3= chrono_types::make_shared<ChNodeFEAxyzrot>(ChFrame<>(ChVector<>(Mvertices[int(facetvec[3])].x(), 
    	...
    	throw chrono::ChException("ERROR opening facet info file: " + std::string(facetFilename) + "\n");
    	exit(EXIT_FAILURE);
    }
	
	/*
	for(int i=0; i<my_mesh->GetNelements()*0; i++){    
	    	auto iel = std::dynamic_pointer_cast<ChElementLDPM>(my_mesh->GetElement(i));
	    	auto nI=std::dynamic_pointer_cast<ChNodeFEAxyzrot>(iel->GetNodeN(0));
	    	auto nJ=std::dynamic_pointer_cast<ChNodeFEAxyzrot>(iel->GetNodeN(1));
	    	auto nK=std::dynamic_pointer_cast<ChNodeFEAxyzrot>(iel->GetNodeN(2));
