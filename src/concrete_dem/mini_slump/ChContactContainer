  ...
  protected:
    ...
    void SumAllContactForces(std::list<Tcont*>& contactlist,
        ...
        for (auto contact = contactlist.begin(); contact != contactlist.end(); ++contact) {
            ...
            ChVector<> force_loc = (*contact)->GetContactForceAbs();			
            ChVector<> force = A * force_loc;	
			//ChVector<> force = force_loc;
			//std::cout<< " force_loc: "<<force_loc<<" force global: "<<force<<std::endl;
            ...
            if (ChBody* body = dynamic_cast<ChBody*>((*contact)->GetObjA())) {				
                torque1 = Vcross(p0 - body->GetPos(), -force);
				//auto Wvel=body->GetFrame_COG_to_abs().GetWvel_loc();
				//double WL=Wvel.Length();
				//std::cout<< "torque1 "<<torque1;
				//if (Wvel!=0){
				//	ChVector<> torque_fric=0.1*torque1* Wvel/Wvel.Length();;
				//	torque1 -= torque_fric;
				//	//std::cout<< " torque_fric "<< torque_fric;
				//}
            }
            ...
            if (ChBody* body = dynamic_cast<ChBody*>((*contact)->GetObjB())) {
                torque2 = Vcross(p0 - body->GetPos(), force);
				//std::cout<< "  torque2 "<<torque2;
				//auto Wvel=body->GetFrame_COG_to_abs().GetWvel_loc();
				//std::cout<<" Wvel "<<Wvel;
				//double WL=Wvel.Length();
				//if (Wvel!=0){
				//	ChVector<> torque_fric=0.1*torque2* Wvel/Wvel.Length();
				//	torque2 -= torque_fric;
				//	//std::cout<< " torque_fric "<< torque_fric <<"\n";
				//}
            }
