...
//
// =============================================================================
// Authors: Alessandro Tasora------> Modified by Bahar Ayhan (24.11.2023)
// =============================================================================

#ifndef CHPARTICLEEMITTER_SMC_H
#define CHPARTICLEEMITTER_SMC_H
...
class ChParticleEmitter_SMC {
  public:
    ...
    ChParticleEmitter_SMC()
        : flow_mode(FLOW_PARTICLESPERSECOND),
		  ...
		  use_aggregate_vol_reservoir(false),
          particle_reservoir(1000),
          mass_reservoir(1),
		  aggregate_vol_reservoir(1),
          created_particles(0),
          created_mass(0),
          off_mass(0),
          off_count(0),
    ...
    void EmitParticles(ChSystem& msystem, double mdt, ChFrameMoving<> pre_transform = ChFrameMoving<>() ) {
        ...
        while (true) {
			...
			if ((use_aggregate_vol_reservoir) && (this->aggregate_vol_reservoir <= 0))
                return;
			...
			this->aggregate_vol_reservoir -= GetAggregateVolume(mbody);
	...
	/// Turn on this to limit on max volume of aggregate particles.
    void SetUseAgregateVolReservoir(bool vl) { this->use_aggregate_vol_reservoir = vl; }
	...
	///
	///
	double& AggregateVolReservoirAmount() { return aggregate_vol_reservoir; }
	...
	/// Turn on this to avoid quantization if generating from an object that has high speed 
    /// in pre_transform (so avoids clusters in jets of particles).
    void SetMortarLayerThickness(double mmortar_layer_h) { mortar_layer_h = mmortar_layer_h; }
	//
	double GetAggregateVolume(std::shared_ptr<ChBody> mbody) {
		double radius=mbody->GetCollisionModel()->GetShapeDimensions(0)[0]-mortar_layer_h;
		//std::cout<<" radius: "<< mbody->GetCollisionModel()->GetShapeDimensions(0)[0] << " mortar_layer_h: "<<mortar_layer_h<<std::endl;
        return (4./3.*CH_C_PI*radius*radius*radius);
    }
  ...
  private:
	...
	double aggregate_vol_reservoir;
	bool use_aggregate_vol_reservoir;

    int created_particles;
    double created_mass;
	
	double mortar_layer_h; // Parameter related to fresh concrete
