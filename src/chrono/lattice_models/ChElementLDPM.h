...
#include "ChElementTetrahedron_6DOFs.h"
...
class ChApi ChElementLDPM : public ChElementTetrahedron_6DOFs,
  ...
  public:
    ...
    virtual std::shared_ptr<ChNodeFEAxyzrot> GetTetrahedronNode(int n) override { return nodes[n]; }

    virtual void SetNodes(std::shared_ptr<ChNodeFEAxyzrot> nodeA,
    ...
    void ComputeAmatrix( Amatrix& A, chrono::ChVector<double>X , chrono::ChVector<double>XI );
    ...
    double ComputeTetVol( ChVector<> p1, ChVector<> p2, ChVector<> p3, ChVector<> p4);
    
    ChMatrixNM<double,6,6> ComputeTetMassN(std::shared_ptr<ChSectionLDPM> facet, ChVector<> pN, ChVector<> pC, ChVector<> pA, ChVector<> pB);
